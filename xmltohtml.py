...
def traducirHTML(archivoXML, archivoHTML):
    ...
    html = "<!DOCTYPE HTML>" + "<html lang='es'>" + \
           ...
           " <meta charset='UTF-8' /> <meta name='author' content='Miguel Suárez Artime'></head>" + \
           "<body>"
    html += "<h1>Arbol genealógico</h1>"
    for element in root.iter():
        # recorre el arbol en profundidad, explorando primero la persona, luego los datos y finalmente sus antecesores
        html += "\n"
        if (element.tag != "personas"):
            if (element.tag == "persona"):
                ...
                html += element.text
                html += "</p>"
            elif (element.tag == "fechaFallecimiento"):
                html += "<h3>Fallecimiento</h3><p>Fecha de fallecimiento: "
                html += element.text
                html += "</p>"
            elif (element.tag == "coordenadasNacimiento"):
                html += "<h4>Coordenadas de nacimiento</h4>"
            elif (element.tag == "coordenadasFallecimiento"):
                html += "<h4>Coordenadas de fallecimiento</h4>"
            ...
            elif (element.tag == "lugarFallecimiento"):
                html += "<p>Lugar de fallecimiento: " + element.text + "</p>"
